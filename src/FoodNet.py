...
class CNN(nn.Module):
    def __init__(self, n_layer, image_size, n_hidden=16, kernel=5, dropout=None, n_classes=101):
        super(CNN,self).__init__()

        self.layers = n_layer

        self.layer1 = nn.Sequential(
            nn.Conv2d(3, n_hidden, kernel_size=kernel, stride=1, padding=2),
        ...
        #     nn.Conv2d(n_hidden, n_hidden*2, kernel_size=kernel, stride=1, padding=2),
        #     nn.BatchNorm2d(n_hidden*2),
        #     nn.ReLU(),
        #     nn.MaxPool2d(kernel_size=2),
        #     )
        # self.layer3 = nn.Sequential(
        #     nn.Conv2d(n_hidden*2, n_hidden*4,  kernel_size=kernel, stride=1, padding=2),
        #     nn.BatchNorm2d(n_hidden*4),
        #     nn.ReLU(),
        #     nn.MaxPool2d(kernel_size=2),
        #     )
        if self.layers > 1:
            for i in range(2,  self.layers+1):
                exec(f'''self.layer{i} = nn.Sequential(
                            nn.Conv2d(n_hidden*(i-1), n_hidden*((i-1)*2), kernel_size=kernel, stride=1, padding=2),
                            nn.BatchNorm2d(n_hidden*((i-1)*2)),
                            nn.ReLU(),
                            nn.MaxPool2d(kernel_size=2),
                            )''')

            out = n_hidden*((self.layers-1)*2)*((image_size//(2**self.layers))**2)
        else:
            out = n_hidden*((image_size//2)**2)
        ...
        if self.layers > 1:
            for i in range(1,  self.layers):
                exec(f'self.fc{i} = nn.Linear(out//div, out//(div*2))')
                div*=2


        exec(f'self.fc{self.layers} = nn.Linear(out//div, n_classes)')
...
class foodNet():
    ...
    def cross_val(self, parameters):
        ...
        for param in combinations:
            ...
            self.model = CNN(n_layer=param['n_layers'],
            ...
            self.best_model = CNN(n_layer=param['n_layers'],
                             image_size = param['resize'],
                             n_hidden = param['n_hidden'],
                             kernel = param['kernel'],
                             dropout=param['dropout'],
